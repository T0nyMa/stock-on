...
- 价格沿 MA5 上行，不破 MA10
- indicators.json → `trend.ma_alignment` 验证

### Step 2: 强势特征
- 连续 3 日涨幅 > 0 或阳线多于阴线（从 kline.json 逐日检查 pct_chg）
- 回调缩量、上涨放量（对比 kline.json 的 volume 与 indicators.json → volume.volume_ratio）
- 主力资金流向配合（如有）
...
### Step 4: 支撑位确认
- MA10 为第一支撑（indicators.json → ma.ma10）
...
用 Write 工具写入 `data/{code}/strategy_bull_trend.json`
