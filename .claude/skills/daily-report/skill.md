...
```
1. Read data/{code}/fundamentals.json → 获取 PE、PB、市值、ROE
...
```
1. Read tracking/{code}-{name}/position.json → 获取持仓成本和策略计划
...
Read `data/market/index.json` + `data/market/sector_scan.json` → 输出大盘日报。
...
4. 调用 `/deploy {date}` — 生成 HTML + 更新首页 + git push
