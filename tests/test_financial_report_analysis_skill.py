...
def test_package_contract():
    ...
    for x in ('最近五年','Phase 0','Phase 10','合并报表','母公司报表','financial_quality_summary','不得输出'):
        assert x in text
    for f in ('evidence-schema','normalization-contract','accounting-comparability','deterministic-rules','scoring-system','issue-card-playbook','risk-control','market-adapters','industry-adapters','report-template','analysis-gaps','research-package'):
...
def test_evidence_first_analysis_contract():
    text=(SK/'SKILL.md').read_text()+(SK/'references/report-template.md').read_text()
    for phrase in ('独立研究资料包','稳定证据编号','结构化数据','管理层解释','外部预测','最强反方','验证指标'):
        assert phrase in text
