...
- Policies: `DATA.QUALITY`, `SEARCH.PRIORITY`, `RESEARCH.EVIDENCE`, `DECISION.SEPARATION`, `PUBLISH.COMPLETE`
- Consumes: `snapshot.quote`, `snapshot.news`, `snapshot.indicators`, `artifact.report_context`, `artifact.tracklist`, `artifact.position`, `artifact.research_summary`, `artifact.financial_collection_status`, `artifact.strategy_scan`
- Produces: `artifact.daily_report`
...
每只持仓和核心观察股必须消费 `artifact.report_context.stocks.{code}.price_volume`，并在对应卡片的“价量结构”行完整渲染 `intraday_volume_ratio`、`volume_vs_ma5`、`volume_vs_ma20`、`recent20_vs_previous20`、`up_down_volume_ratio_90d`、`mfi14`（MFI）、`cmf20`（CMF）、`obv_20d_direction`（OBV）、`price_volume_label`、`interpretation_flags` 和 `evidence_gaps`。`interpretation_flags` 与 `evidence_gaps` 必须按 producer 返回的列表顺序逐项原值展示；空列表确定性显示为 `[]`，不得自行翻译、套用公式、合并或猜测。任何缺失字段必须保留为 `unavailable`，不得省略整行、补算或猜测。
...
- A/H 两地指标必须分开取证；H 股缺失字段不得用 A 股指标替代。
