...
每只持仓和核心观察股必须消费 `artifact.report_context.stocks.{code}.price_volume`，并在对应卡片的“价量结构”行完整渲染 `intraday_volume_ratio`、`volume_vs_ma5`、`volume_vs_ma20`、`recent20_vs_previous20`、`up_down_volume_ratio_90d`、`mfi14`（MFI）、`cmf20`（CMF）、`obv_20d_direction`（OBV）和 `price_volume_label_and_interpretation`。任何缺失字段必须保留为 `unavailable`，不得省略整行、补算或猜测。
