...
读 `data/{code}/` 下 SQLite 日K查询结果, SQLite 指标快照（需 60 日数据）

## 分析框架

### Step 1: 分型识别
- 从 SQLite 日K查询结果 中找顶分型和底分型
...
- 从 SQLite 指标快照 读取 macd.hist 数据，与原价格高低点对比
...
用 写入 工具写入 `data/{code}/strategy_chan_theory.json`
