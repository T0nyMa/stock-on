...
- [x] Write failing contract tests for collection-only Stage A, EasyAnySearch, bilingual queries, freshness, gate status, provisional outputs, latest-call/forecast/regulation/media/peer requirements.
- [x] Run focused tests and confirm contract failure.
- [x] Implement the recent-first workflow and report changes in Skill references.
- [x] Run focused tests and Skill validation.
...
- [x] Write failing tests for 30/90-day freshness boundaries, corrected-version staleness, missing latest-report blocking, missing call partial status, coverage score, and allowed output.
- [x] Implement `classify_freshness`, `validate_payload`, `evaluate_gate`, and JSON CLI.
- [x] Verify current weak media remains a source item but cannot satisfy an official-report requirement.
- [x] Run focused tests.
...
- [x] Write failing tests for `financial_collection_status` validation, HK round trip, and CLI access.
- [x] Add snapshot kind and `load_financial_collection_status`.
- [x] Require deep research to consume only pass-gated summaries; reports surface pending forecast-verification events.
- [x] Run focused tests.
...
- [x] Run Skill validation, focused tests, full `pytest -q`, and `git diff --check`.
- [x] Mark plan complete and commit `feat: require recent-first financial collection`.
- [x] Merge locally to main, rerun full verification, and clean the owned worktree.
